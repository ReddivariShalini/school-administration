...
def write_into_csv(info_list):
  with open('student_info.csv','a',newline='') as csv_file:
    ...
    if csv_file.tell()==0:
        writer.writerow(["name","age","contact_number","emailid"])
...
if __name__ =='__main__':
    ...
    while(condition):
        ...
        choice_check=input("is the entered information is correct or not (yes/no")
        if choice_check == "yes":
            write_into_csv(student_info_list)
            condition_check = input("enter yes/no to enter information for other students:")
